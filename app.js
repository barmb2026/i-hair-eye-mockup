...
const topCopy = variantKey === "a" ? {
  ...
  catch: "〈京都駅徒歩5分〉作り込みすぎない抜け感とトレンド。顔周りレイヤーも似合う形へ",
  message: "今の雰囲気は好き。でも少しだけ変えたい。そんなときも気軽にご相談ください。髪質や顔型、朝の過ごし方まで聞きながら、頑張りすぎなくても形になるスタイルを一緒に探します。透明感カラー、顔周りレイヤー、ショート、髪質改善、メンズまで担当。半個室でゆっくり過ごせて、価格も無理なく通える設定です。",
} : {
  label: "B案｜コンセプト改修",
  name: "_i hair&eye【アイ】京都駅/レイヤーカット/メンズパーマ/髪質改善",
...
if (variantKey === "a") {
  commonSearchWords = "京都駅/カラー/髪質改善/レイヤーカット/バタフライレイヤー/メンズパーマ/フェザーパーマ/シャドウパーマ";
  features.splice(0, features.length,
    ...
    { category: "一人ひとりに似合うヘアを提案してくれるサロン", image: "assets/105cc1ff3ac1c3f3.jpg", title: "顔周りレイヤーも、好きと苦手を聞いて似合う形へ", body: "流行をそのまま入れず、前髪や結ぶ日の顔周りまで見て形を決めます。", coupon: "カウンセリングからメニューをご案内", price: "¥0" },
  ...
  commitmentPages[0] = {
    ...
    steps: [
      ...
      ["作り込みすぎない、自然な抜け感", "顔周りレイヤーも入れすぎず、巻かない日や髪を結ぶ日まで自然に。頑張りすぎなくても雰囲気が残る形を考えてカットします。"],
...
document.querySelector("#pattern-summary-copy").textContent = variantKey === "a"
  ? "現行のナチュラル＋トレンド、価格、幅広い技術を残し、レイヤーなど今のトレンドも自然に加える案です。"
...
const priorityCouponCopy = {
  2: ["【新規人気No.1】カット＋リタッチカラー＋Aujua 3step", "フルカラーは+¥1,100。初めての方の定番入口。"],
  1: ["【学割U24】カット＋リタッチカラー＋Aujua 3step", "フルカラーは+¥1,100。24歳以下の学生向け。"],
  10: ["【何度でも】カット＋リタッチカラー＋Milbon 3stepTR", "フルカラーは+¥1,100。新規も再来も使える定番。"],
  93: ["【新規】似合わせカット＋Aujua 3stepケア", "短くする不安や顔周りも相談できるカット入口。"],
  92: ["【学割U24】似合わせカット＋Aujua 3stepケア", "24歳以下の学生向け。顔周りも相談できます。"],
  8: ["【顔周りも相談】カット＋イルミナorアディクシーカラー", "赤みや色落ちまで相談できる上質カラー。"],
  16: ["【メンズ人気】似合わせカット＋スパor眉カット", "清潔感と朝の扱いやすさを整えるメンズ入口。"],
  17: ["【メンズ】カット＋リタッチカラー＋TR＋眉カット", "フルカラーは+¥1,100。眉までまとめて整えます。"],
  3: ["【癖・広がり相談】髪質改善メニューを来店後に決定", "癖を伸ばすか手触りを整えるか、髪を見て決定。"],
  28: ["【髪質改善】カット＋髪質改善カラー", "カラー後の広がりや手触りが気になる方へ。"],
};

const priorityCouponNumbers = [2, 1, 10, 93, 92, 8, 16, 17, 3, 28];

function improvedCoupon(coupon, index) {
  const sourceNo = index + 1;
  const priority = priorityCouponCopy[sourceNo];
  const title = fitUnits(priority?.[0] || `${couponIntent(coupon)}${cleanCouponTitle(coupon.title)}`, 36);
  const lead = priority?.[1] || benefitLead(coupon);
  const description = fitUnits(`${lead}/${commonSearchWords}`, 90);
  return { ...coupon, sourceNo, improvedTitle: title, improvedDescription: description, improvedCondition: couponCondition(coupon) };
}

function couponOrderGroup(coupon) {
  const priorityIndex = priorityCouponNumbers.indexOf(coupon.sourceNo);
  if (priorityIndex >= 0) return priorityIndex;
  const source = `${coupon.title} ${coupon.description} ${coupon.prefix.join(" ")}`;
  if (/選択できません|line up/i.test(source)) return 900;
  if (/birthday|誕生日|再\s*来/i.test(source)) return 800;
  if (/スタイリスト指定|当日限定|櫟原限定/.test(source)) return 700;
  if (/ヘアセット/.test(source)) return 600;
  if (/Men|men's|メンズ/i.test(source)) return 300;
  if (/髪質改善|縮毛矯正|ストレート/.test(source)) return 400;
  if (/ブリーチ|インナー|ハイライト|グラデーション/.test(source)) return 500;
  if (/TOKIO|Aujua|oggiotto/.test(source)) return 550;
  if (/カット|カラー/.test(source)) return 200;
  return 650;
}

function improvedCouponsInOrder() {
  const all = window.COUPONS.map((coupon, index) => improvedCoupon(coupon, index));
  all.sort((a, b) => couponOrderGroup(a) - couponOrderGroup(b) || a.sourceNo - b.sourceNo);
  all.forEach((coupon, index) => { coupon.recommendedOrder = index + 1; });
  return all;
}

function filteredCoupons() {
  const all = improvedCouponsInOrder();
  if (currentFilter === "all") return all;
  return all.filter(c => couponFilterType(c)[currentFilter]);
}

function couponMigrationCopy() {
  return [
    "推奨順\t現行No\t現在のクーポン名\t修正後クーポン名\t価格\t修正後説明\tその他条件",
    ...improvedCouponsInOrder().map(c => [
      c.recommendedOrder,
      `#${String(c.sourceNo).padStart(2, "0")}`,
      c.title,
      c.improvedTitle,
      c.price,
...
function renderCoupons() {
  ...
  document.querySelector("#coupon-list").innerHTML = shown.map(c => {
    ...
    return `<article class="coupon-card">
      ...
      <div class="coupon-main">
        <div class="coupon-edit-key">推奨順 ${c.recommendedOrder}｜現行 #${String(c.sourceNo).padStart(2, "0")} を編集</div>
        <details class="coupon-source"><summary>現在の登録名を確認</summary><p>${escapeAttribute(c.title)}</p></details>
  ...
  document.querySelector("#coupon-copy-plan").dataset.copyValue = couponMigrationCopy();
}
...
function buildFullPageCopy() {
  const salonCopy = [
    ...
    "",
    "【クーポン96件】",
    ...improvedCouponsInOrder().map(c => `${c.recommendedOrder}. ${c.improvedTitle}\n現行 #${String(c.sourceNo).padStart(2, "0")}：${c.title}\n${c.price}\n${c.improvedDescription}\n来店日条件：指定なし\nその他条件：${c.improvedCondition}`),
    "",
    "【スタイル12件】",
